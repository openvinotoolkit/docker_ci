...
FROM openvino/ubuntu18_dev_no_samples:2021.4.1
...
RUN useradd ${USER_NAME} -l -u ${USER_ID} -g ${GROUP_ID} -ms /bin/bash  && \
...
COPY --chown=workbench workbench ${OPENVINO_WORKBENCH_ROOT}/
