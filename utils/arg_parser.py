...
from utils.utilities import check_internal_local_path, check_printable_utf8_chars
...
def parse_args(name: str, description: str):  # noqa
    ...
    gen_dockerfile_subparser = subparsers.add_parser(
        "gen_dockerfile", help="Generate a dockerfile to " "dockerfiles/<image_os> folder"
    )
    ...
    if (
        args.mode != "deploy"
        and args.package_url
        and args.source == "local"
        and not args.package_url.startswith(("http://", "https://", "ftp://"))
    ):
    ...
    if args.mode in ("gen_dockerfile", "build", "build_test", "all"):
        ...
        if not args.distribution and args.package_url:
            if "_runtime_" in args.package_url:
                ...
                parser.error(
                    f"Cannot get distribution type from the package URL provided. {args.package_url} "
        ...
        if not args.package_url and args.distribution not in ("base",):
            ...
            if args.mode != "gen_dockerfile" or args.rhel_platform == "autobuild":
                ...
                with contextlib.suppress(KeyError):
                    args.package_url = INTEL_OPENVINO_VERSION[args.product_version][args.os][
                        args.distribution
                    ]
                if not args.package_url:
                    parser.error(
                        f"Cannot find package url for {args.product_version} version "
